...
cp -r "${SRC}"/packages/blobs/desktop/debian/xdg-lxqt "${destination}"/etc/riscv
...
cat <<EOF > "${destination}"/usr/share/lxqt-background-properties/riscv.xml
<?xml version="1.0"?>
<!DOCTYPE wallpapers SYSTEM "lxqt-background-properties-wp-list.dtd">
<wallpapers>
